...
const History = ({ theme, ipAddress: propIpAddress }) => {
  const [transactions, setTransactions] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [searchParams] = useSearchParams();
  const [ipAddress, setIpAddress] = useState(propIpAddress || '192.168.98.252');

  useEffect(() => {
    // Extract IP address from URL query parameter if present
    const ipFromUrl = searchParams.get('ip');
    if (ipFromUrl) {
      setIpAddress(ipFromUrl);
    } else if (propIpAddress) {
      setIpAddress(propIpAddress);
    }
  }, [searchParams, propIpAddress]);
